...
int main (int argc, char** argv) {
  ...
  google::InitGoogleLogging("Proton Reconstruction");
  ...
  SKColorScheme();
   ...
   /* ------- Reading Root Data -------- */
  TString fileList = "../SoKAI/macros/quasifree_reconstruction/files/p_p_500AMeV_nn.root";
  ...
  Int_t nSamples_0 = 0, nSamples_1 = 0, nSamples_2 = 0;

  Int_t eventCounter = 0;
  Int_t acceptedEvents = 0;

  int nTrainingSize   = (7.0/10.0)*nSamples*2;
  int nTestSize       = (3.0/10.0)*nSamples*2;


  while ( (nSamples_0 < nSamples || nSamples_1 < nSamples) && eventCounter < nEvents ){
   ...
   cout<<"nSamples_0 : "<<nSamples_0<<" "<<" nSamples_1 : "<<nSamples_1<<" nSamples_2 : "<<nSamples_2<<endl;
  ...
  model->SetSummaryFile("summary_proton_reconstruction",argv[12]);
...
string weightfilename = "weights_protons_";
weightfilename = weightfilename + argv[12] + ".txt";


model->SaveWeights(weightfilename);
...
TCanvas *reso_canvas = new TCanvas("reso_canvas","Resolution Canvas");
 ...
 reso_canvas->cd(2);
  ...
  TString filename = "training_results_regression_protons";
